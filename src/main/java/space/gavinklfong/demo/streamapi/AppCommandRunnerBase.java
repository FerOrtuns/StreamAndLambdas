...
import space.gavinklfong.demo.streamapi.models.Copy;
...
public class AppCommandRunnerBase implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<Order> pedidosContieneProductosBaby = new ArrayList<>();


        for (var order : orders) {
            for (var product : order.getProducts()) {
                if (product.getCategory().equalsIgnoreCase("BABY")) {
                    pedidosContieneProductosBaby.add(order);
                    break;
                }
            }
        }
        ...
        result.forEach(order -> System.out.println(order.getId()));


		/*Exercise 3 — Obtain a list of products (clones of the original products) with category = “Toys” and then apply
		10% discount*/
        ...
        List<Product> result2 = productRepos.findAll()
                .stream()
                .filter(product -> product.getCategory().equalsIgnoreCase("Toys"))
                .map(product -> product.cloneWithPrice(product.getPrice() * 0.9))
                .toList();


        List<Copy> resultCopy = productRepos.findAll()
                .stream()
                .filter(product -> product.getCategory().equalsIgnoreCase("Toys"))
                .map(product ->
                        new Copy(product, product.cloneWithPrice(product.getPrice() * 0.9)))
                .toList();

        System.out.println();
        resultCopy.forEach(copy -> System.out.println(copy));
        System.out.println();
...
/*
       ...
       // Exercise 5 — Get the cheapest product of “Books” category
        ...
        var resultadExercise05 = productRepos.findAll()
                .stream()
                        .filter(product -> product.getCategory().equalsIgnoreCase("books"))
                                .min(comparing(Product::getName));

        // Exercise 6 — Get the 3 most recent placed order

        // var resultadExercise06 =
